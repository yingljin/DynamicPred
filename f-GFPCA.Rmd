...
# Try on subject 1
pred20 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 20, df_pred_unique, subj=2),
                     type = "up to 195") %>% 
  filter(bin>195)


pred40 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 40, df_pred_unique, subj=2),
                     type = "up to 395") %>%
  filter(bin>395)


pred60 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 60, df_pred_unique, subj=2),
                     type = "up to 595") %>%
  filter(bin>595)
...
# Try on subject 1
pred20 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 20, df_pred_unique, subj=3),
                     type = "up to 195") %>% 
  filter(bin>195)


pred40 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 40, df_pred_unique, subj=3),
                     type = "up to 395") %>%
  filter(bin>395)


pred60 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 60, df_pred_unique, subj=3),
                     type = "up to 595") %>%
  filter(bin>595)


df_pred_latent %>% filter(id==3 & sind_inx==bin) %>%
  select(bin, eta_i) %>% mutate(type="true") %>%
  add_row(pred20) %>%
  add_row(pred40) %>% 
  add_row(pred60) %>% 
...
# Try on subject 1
pred20 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 20, df_pred_unique, subj=4),
                     type = "up to 195") %>% 
  filter(bin>195)


pred40 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 40, df_pred_unique, subj=4),
                     type = "up to 395") %>%
  filter(bin>395)


pred60 <- data.frame(bin = mid, 
                     eta_i = in_samp_dyn_pred(fpca_fit, 60, df_pred_unique, subj=4),
                     type = "up to 595") %>%
  filter(bin>595)


df_pred_latent %>% filter(id==4 & sind_inx==bin) %>%
  select(bin, eta_i) %>% mutate(type="true") %>%
  add_row(pred20) %>%
  add_row(pred40) %>% 
  add_row(pred60) %>% 

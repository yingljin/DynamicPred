...
This is pretty much a linear functions. Is that supposed to happen? 

```{r, class.source = "fold-show"}
new_mu <- predict(debias_glmm, type = "terms")[, 1]
new_mu <- unique(new_mu)
plot(unique(train_df$t), new_mu, type = "l")
```
...
- The scaled and unscaled parameters seem to make a difference in prediction, which they shouldn't 

```{r, fig.height=8, fig.width=8}
par(mfrow=c(2, 2))
plot(df_pred$pred0.2, df_pred3$pred0.2, pch=20, cex = 0.5, 
     xlab="unscaled", ylab="scaled", main = "Up to 0.2")
plot(df_pred$pred0.4, df_pred3$pred0.4, pch=20, cex = 0.5,
     xlab="unscaled", ylab="scaled", main = "Up to 0.4")
plot(df_pred$pred0.6, df_pred3$pred0.6, pch=20, cex = 0.5,
     xlab="unscaled", ylab="scaled", main = "Up to 0.6")
plot(df_pred$pred0.8, df_pred3$pred0.8, pch=20, cex = 0.5,
     xlab="unscaled", ylab="scaled", main = "Up to 0.8")
```
...
The model fitting took about 6 minutes at this data scale, which obviously is not realistic for 500 simulated datasets. 
...
```{r}
df_test_sub <- test_df %>% filter(id %in% 1:10 & sind==bin)
...
colnames(pred_adglmm_all) <- c(colnames(pred_adglmm_all)[1:15], "pred1", "pred2", "pred3", "pred4")
```

...
- The bam model below couldn't do out-of-sample prediction
- It also take a long time to fit

```{r,class.source='show', eval=FALSE}
# try to fit a flexible GLMM
t1 <- Sys.time()
fit_glmm_sp <- bam(Y ~ s(t, bs="cr", k=10)+ti(id, t, bs=c("re", "cr"), k = c(5, 5), mc = c(TRUE, FALSE)), 
                   family = binomial, data=train_df, 
                   method = "fREML",
                   discrete = TRUE)
t2 <- Sys.time()
...
- I should construct basis functions and fit model with GLMMadaptvie::mixed_model instead

### GLMMadaptive + basis functions

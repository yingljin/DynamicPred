...
ggarrange(p1, p3, nrow = 1, common.legend = T)
```
...
ise <- full_join(tb1, tb3, by = "Window")
...
## Investigate a bit more on the difference in prediction

When using the unscaled estimates of eigenvalues and eigenfunctions, the Laplace Approximation does not converge all the time (convergence rate = `r round(mean(converge_vec), 2)`). And when using scaled estimates, the convergence rate is even lower (`r round(mean(converge_vec2), 2)`). 

When I increasing the maximum iteration to 1000, the predictive performance from two sets of estiamtes got very similar. However, the convergence rate is still very different. Below I try to show a example of the discrepancy in prediction. As we see, the most problematic situation is when observations is up to 0.4.

```{r}
# subjects with higher deiviation 
# subjects with greatest difference
id_diff <- df_pred %>% select(id, t, pred0.4) %>%
  full_join(df_pred3 %>% select(id, t, pred0.4), by = c("id", "t")) %>% 
  group_by(id) %>%
  summarise(diff = mean((pred0.4.x-pred0.4.y)^2, na.rm = T)) %>% 
  arrange(desc(diff))

# subject 43, 79, 57, 148 have the most different prediction
df_pred %>% filter(id == id_diff$id[1]) %>% 
  select(t, starts_with("pred")) %>%
  pivot_longer(starts_with("pred")) %>%
  left_join(df_pred3 %>% filter(id == id_diff$id[1]) %>%
              select(t, starts_with("pred")) %>%
              pivot_longer(starts_with("pred")),
            by = c("t", "name")) %>%
  ggplot()+
  geom_point(aes(x=value.x, y=value.y), na.rm = T, size = 0.2)+
  facet_wrap(~name)+
  labs(title = paste0("Subject ", id_diff$id[1]), x="Unscaled", y="Scaled")
```

Think about the estimates. The scaled estimates of eigenvalues are much smaller than the unscaled once, meaning the prior distribution of score using scaled estimates would be much more centered thant the unscaled ones. This would also drive the posterior distribution to be in similar shapes. Since the distribution from scaled eigenvalues has such high density at certain values, could it be that the sampling quickly got trapped in a small interval that is far from the true optimal values? 

Take the same subject with observation up to 0.4 as an example. Below is the iterative history until convergence. In this case, unscaled estiamtes converged at the 67 iteration and the scaled one did not converge. 

```{r, results='hide'}
this_id <- id_diff$id[1]
this_t <- 0.4
...
Let's zoom in on the later part of the scaled parameter. It looks like sampling bounced symmetrically around a certain line. 
...
Fit_scaled$History %>% as.data.frame() %>%
  ...
  labs(title = "Scaled parameter, after 100th iteration")
```


```{r converge_strata, eval=FALSE}
# Look at prediction results stratified by convergence status
converge_vec
converge_vec2
```
...
err1_conv$window = cut(err1_conv$t,
                       breaks = seq(0, 1, by = 0.2), include.lowest = T)
# table(err1$window)
err1_conv <- split(err1_conv, f = err1_conv$window)
err1_conv[[1]]
...
- Here we can fit a model with random intercept and slope for time

$$g(E(Y_i(t))) = \beta_0+\beta_1t+b_{i0}+b_{i1}t$$
- But this model is obviously too simple for the data generation scheme


```{r}
# model fit
t1 <- Sys.time()
fit_adglmm <- mixed_model(Y ~ t, random = ~ t | id, data =  train_df, family = binomial())
t2 <- Sys.time()
# t2-t1
```
...
- Fit on all 500 subjects take too long. I reduced the sample size to 10 
- In fact, 10 samples took more than 30 minutes to fit the model. Therefore, I further reduce the number of observations for each subject. I'll start with only the 1/10 of the time track (100 observations), take observations at bin midpoints 5, 15, 25, ...., 995. This is to preserve the non-linearity of latent function when reducing grid density

$$g(E(Y_i(t))) = \sum_{k=1}^5\zeta_{k}B_k(t)+\sum_{l=1}^4\xi_{il}\phi_l(t)$$

```{r}
# subset 10 subjects from training dataset 
df_train_sub <- train_df %>% filter(id %in% 1:10 & sind==bin)
df_train_sub$id <- droplevels(df_train_sub$id)
# levels(df_train_sub$id)
# intersect(t, unique(df_train_sub$t))
# View(df_train_sub)
```


- Below I construct basis functions for the fixed effect/random intercept. Let's use five basis functions to start with. I believe this is far less flexible than the FPCA model, but it still takes a really long time to run. It is actually not realistic to repeat 500 times. 

PS 50 subjects took 1.06 hours to fit with the same model structure. 


```{r}
# construct design matrix for the fixed effect intercept
t_sub <- unique(df_train_sub$t)
# fix_xmat <- smoothCon(s(t, bs="cr", k=5), data = data.frame(t=t_sub))
fix_xmat <- ns(t_sub, df=5)
# dim(fix_xmat[[1]]$X)

# basis functions
fix_xmat <- data.frame(t=t_sub, 
                       fix_xmat)
colnames(fix_xmat) <- c("t", paste0("B", 1:5))

df_train_sub <- df_train_sub %>% left_join(fix_xmat, by = "t")
```


- Below I construct basis functions for the individual random effect (also a functinal intercept), with four basis functions to start with.


```{r}
# construct design matrix for the random effect
rand_xmat <- smoothCon(s(t, bs="cr", k=4), data = data.frame(t=t_sub))
# rand_xmat <- ns(t_sub, df = 4)

# dim(rand_xmat[[1]]$X) # 5 basis functions for each of the 50 subjects
rand_xmat <- data.frame(t = t, rand_xmat[[1]]$X)

df_train_sub <- df_train_sub %>%
  left_join(rand_xmat, by = c("t"))
```


- Now I'll fit the model and record the time spent on model fitting

The model fitting took about 12 minutes at this data scale, which obviously is not realistic for 500 simulated datasets. 

Also tried increasing number of basis functions, which ended up with "computationally singular". In fact, this method does not seem very numerically stable. 

```{r, eval=TRUE, class.source = "fold-show"}
# model fit
t1 <- Sys.time()
fit_adglmm <- mixed_model(fixed = Y ~ 0+B1+B2+B3+B4+B5, 
                          random = ~X1+X2+X3+X4 | id,
...
- But anyways, let's try to use this method for prediction. Let's do that for 10 new test subjects. 


```{r}
df_test_sub <- test_df %>% filter(id %in% 1:10 & sind %in% mid)
df_test_sub$id <- droplevels(df_test_sub$id)
# levels(df_test_sub$id)

# basis functions for both fixed and random effects should be shared across sample
df_test_sub <- df_test_sub %>% left_join(fix_xmat, by = "t") %>%
  left_join(rand_xmat, by = "t")

# given observation from 0.2, 0.4, 0.6, 0.8, predict the rest
# this step in fact is very fast
pred_adglmm1 <- predict(fit_adglmm, 
                      newdata = df_test_sub %>% filter(t<=0.2),
                      newdata2 =  df_test_sub %>% filter(t>0.2),
                      type = "subject_specific", type_pred = "link",
                      return_newdata = TRUE)

pred_adglmm2 <- predict(fit_adglmm, 
                      newdata = df_test_sub %>% filter(t<=0.4),
                      newdata2 =  df_test_sub %>% filter(t>0.4),
                      type = "subject_specific", type_pred = "link",
                      return_newdata = TRUE)

pred_adglmm3 <- predict(fit_adglmm, 
                      newdata = df_test_sub %>% filter(t<=0.6),
                      newdata2 =  df_test_sub %>% filter(t>0.6),
                      type = "subject_specific", type_pred = "link",
                      return_newdata = TRUE)

pred_adglmm4 <- predict(fit_adglmm, 
...
```{r}
pred_adglmm_all <- df_test_sub %>% 
  left_join(pred_adglmm1$newdata2 %>% select(id, t, pred), by = c("id", "t")) %>% 
  left_join(pred_adglmm2$newdata2 %>% select(id, t, pred), by = c("id", "t")) %>%
  left_join(pred_adglmm3$newdata2 %>% select(id, t, pred), by = c("id", "t")) %>%
  left_join(pred_adglmm4$newdata2 %>% select(id, t, pred), by = c("id", "t"))

colnames(pred_adglmm_all) <- c(colnames(pred_adglmm_all)[1:14], "pred1", "pred2", "pred3", "pred4")
```
...
pred_adglmm_all %>%
  ...
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=eta_i, col = "True"))+
  geom_line(aes(x=t, y=pred1, col = "0.2"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred2, col = "0.4"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred3, col = "0.5"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred4, col = "0.8"), linetype="dashed", na.rm=T)+
  facet_wrap(~id)+
...
Perhaps I should try to verify my thoughts about laplace approximation by different generation scheme. For example, what if the true eigenvalues are on larger scale? 

- Well, after increasing the scale of the true eigenvalues, the convergence rate did get closer between scaled and unscaled parameter. Also reversed as I expected.
- But on second though, A more effecient way is to calcualted ISE and AUC stratified by convergence status

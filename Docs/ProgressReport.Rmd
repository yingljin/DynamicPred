...
Let's first try to re-do the who local GLMM step using glmmTMB. Looks like no numeric issue occurred. It takes slightly shorter time (about 1 minute) than PIRLS in glmer 
...
- Time: `r round(t_tmb, 2)` minutes
- Mean/Intercept function: It looks like glmmTMB estimates lie right between PIRLS and AGQ! It takes slightly shorter time than PIRLS (`r round(t_pirls, 2)` minutes) and mean function shoots more downward, not as bad as AGQ though. 
...
df %>% 
  ...
  ggplot()+
  geom_line(aes(x=sind, y=eta_hat, col = "glmer"))+
  geom_line(aes(x=sind, y=eta_hat_tmb, col = "glmmTMB"))+
...
df %>% 
  ...
  left_join(df_est_latent_tmb %>% select(id, bin, eta_hat) %>% rename(eta_hat_tmb=eta_hat), 
            by = c("id", "bin")) %>%
  ggplot()+
  geom_line(aes(x=bin, y=eta_hat, col = "glmer"))+
  geom_line(aes(x=bin, y=eta_hat_tmb, col = "glmmTMB"))+
  geom_point(aes(x=bin, y = num), size = 0.5)+
  facet_wrap(~id, scales = "free")+
  labs(x = "Time", y = "Estimated latent function")
```

So it looks like glmmTMB still overshoots downwards. Between the two methods, the mid-day part (active part for most people) seems to be very similar. Difference lies mostly at both ends, with consecutive inactive minutes. 
...
Hmm...Compared to conditional intercept, FPCA seems to shoot the population mean downward in PIRLS but upward for glmmTMB. Does that make sense? 
...
- Used PIRLS for local GLMMs step
...
- `r round(t_local_glmm, 2)` minutes spent on fitting local GLMMs, `r round(t_fpca, 2)` seconds on FPCA model, `r round(t_pred, 2)` minutes on out-of-sample prediction, `r round(t_interp, 2)` minutes on interpolation to the original grid 
...
- We can either assume that the latent function value is constant within the same bin, or use linear interpolation to fill in the gaps between bin midpoints. Here I wanna compare if this choice would make any different. Also, there are a lot of other methods to fill in the gaps, such as projection. 
- For linear interpolation, I set the edges to be the closest data extreme.
- The interpolation itself spent `r round(t_interp, 2)` minutes.
...
- Below is the model fit on the trainign set
...
```{r}
# time
t_fit <- c(t_local_glmm+t_fpca, t_local_glmm+t_fpca, t_est_adglmm)
units(t_fit) <- "mins"

t_test <- c(t_pred, t_pred+t_interp, t_pred_adglmm)
# units(t_pred)

t_total <- t_fit+t_test

idx1 <- idx2 <- idx3 <- c(1, 3, 3, 3)

...
Since we are assuming value of latent function is constant within a bin, why can't we just do the same when calculating AUC without interpolation? 

Remove the three subjects with failed approximation



```{r}
lst_auc <- df %>% 
  filter(id %in% unique(df_test$id) & !id %in% skip_id ) %>%
...
tb_auc %>%
  ...
  add_header_above(c("Time spent on model fitting" = 1, "3.3" = 3, "20.72" = 3), bold = F) %>%
  add_header_above(c("Time spent on prediction" = 1, "20.0" = 3, "12.2" = 3), bold = F) %>% 
  add_header_above(c(" "=1, "fGPFCA"=3, "GLMMadaptive"=3))
```

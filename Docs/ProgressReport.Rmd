...
```{r}
issue_fit <- glmer(Y ~ 1 + (1|id), data = df_bin_lst[['465']],
                             family = binomial)
summary(issue_fit)


# table(df_bin_lst[['5']]$id, df_bin_lst[['5']]$Y)
```

```{r eval=FALSE}
colSums(table(df_bin_lst[['5']]$id, df_bin_lst[['5']]$Y)==0)
colSums(table(df_bin_lst[['465']]$id, df_bin_lst[['465']]$Y)==0)
```

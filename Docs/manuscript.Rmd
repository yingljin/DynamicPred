...
# Small-scale simulation: one iteration
...
- Another way is the simple reduce the measurement density. For example, instead of every minute, we can use every 10 minutes. In this case, the binning step in fGFPCA would bin every 100 minutes in to one bin. 
- - When we have smaller density of measurement grid, both methods could run into numeric issues in both methods. For example, singularity happened the the local GLMM step of fGFPCA procedure. This might be able to get resoved by changing the optimizer (e.g. Nealder_Mead). Do we want to do that? Or perhaps we simply want to use the full track? 

- We may also need to reduce number of simulations (GLMMadaptive took 20-ish minutes for one simulaiton).
...
```{r fig_pred_subset}
fig_fgfpca_subset <- df_pred_m_fGFPCA %>% filter(id %in% 501:504) %>%
  mutate_at(vars(eta_i, pred0.35, pred0.45, pred0.55, pred0.65), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=eta_i, col = "True"))+
  geom_line(aes(x=t, y=pred0.35, col = "0.35"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred0.45, col = "0.45"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred0.55, col = "0.55"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred0.65, col = "0.65"), linetype="dashed", na.rm=T)+
  facet_wrap(~id)+
  labs(title = "fGFPCA", col = "Maximum observation time")

fig_adglmm_subset <- df_pred_m_adglmm %>% filter(id %in% 501:504) %>%
  mutate_at(vars(eta_i, pred0.35, pred0.45, pred0.55, pred0.65), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=eta_i, col = "True"))+
...
```{r ise_fGFPCA_subset}
window2 <- seq(0.25, 0.75, by = 0.1)
ise_mat_subset <- df_pred_m_fGFPCA %>%
    mutate(err1 = (pred0.35-eta_i)^2,
           err2 = (pred0.45-eta_i)^2,
           err3 = (pred0.55-eta_i)^2,
           err4 = (pred0.65-eta_i)^2) %>%
    select(id, t, starts_with("err")) %>% 
    mutate(window = cut(t, breaks = window2, include.lowest = T)) %>% 
    group_by(window, id) %>% 
    summarise_at(vars(err1, err2, err3, err4), sum) %>% 
    group_by(window) %>% 
    summarize_at(vars(err1, err2, err3, err4), mean) %>%
    ...
    select(starts_with("err"))
colnames(ise_mat_subset) <- c("(0.25,0.35]", "(0.25,0.45]", "(0.25,0.55]", "(0.25,0.65]")
```

```{r ise_GLMMadaptive_subset}
ise_mat_subset2 <- df_pred_m_adglmm %>%
    mutate(err1 = (pred0.35-eta_i)^2,
           err2 = (pred0.45-eta_i)^2,
           err3 = (pred0.55-eta_i)^2,
           err4 = (pred0.65-eta_i)^2) %>%
    ...
    filter(window != "[0.25,0.35]") %>% 
    select(starts_with("err"))
colnames(ise_mat_subset2) <- c("(0.25,0.35]", "(0.25,0.45]", "(0.25,0.55]", "(0.25,0.65]")
```


```{r}
data.frame(Window = c("(0.35,0.45]", "(0.45,0.55]", "(0.55,0.65]", "(0.65, 0.75]"),
           ise_mat_subset, ise_mat_subset2, check.names = F) %>%
  kable(digits = 3, table.attr = "style = \"color: black;\"", caption = "Integrated squared error") %>%
  kable_styling(full_width = F) %>% 
  add_header_above(c(" "=1, "fGFPCA" = 4, "GLMMadaptive" = 4)) %>%
...
```{r auc_fGFPCA_subset}
auc_mat_subset <- df_pred_m_fGFPCA %>%
  mutate(window = cut(t, breaks = window2, include.lowest = T)) %>% 
  select(Y, starts_with("pred"), window) %>%
  group_by(window) %>%
...
colnames(auc_mat_subset) <- c("(0.25,0.35]", "(0.25,0.45]", "(0.25,0.55]", "(0.25,0.65]")
```

```{r auc_GLMMadaptive_subset}
auc_mat_subset2 <- df_pred_m_adglmm %>%
  mutate(window = cut(t, breaks = window2, include.lowest = T)) %>% 
  select(Y, starts_with("pred"), window) %>%
  group_by(window) %>%
  summarise(auc1 = get_auc(Y, pred0.35),
...
colnames(auc_mat_subset2) <- c("(0.25,0.35]", "(0.25,0.45]", "(0.25,0.55]", "(0.25,0.65]")
```

```{r}
data.frame(Window = c("(0.35,0.45]", "(0.45,0.55]", "(0.55,0.65]", "(0.65, 0.75]"),
           auc_mat_subset, auc_mat_subset2, check.names = F) %>%
  kable(digits = 3, table.attr = "style = \"color: black;\"", caption = "Integrated squared error") %>%
  kable_styling(full_width = F) %>% 
  add_header_above(c(" "=1, "fGFPCA" = 4, "GLMMadaptive" = 4)) %>%
  add_header_above(c(" "= 1, "Observation track" = 8))
...
We take 80% (7010) subjects for training, 20% (1753) for out-of-sample prediction. 

In the GLMMadaptive model, I could only fit a model with random intercept. It took more than 20 minutes for model fitting. 
...
df_nhanes %>% select(SEQN, Z, sind) %>% 
  ...
  mutate(SEQN = factor(SEQN, labels = 1:8763)) %>%
  ggplot()+
...
```{r auc_fGFPCA_nhanes}
window3 <- seq(0, 1440, by = 240)

auc_mat_nhanes <- nhanes_pred_fgfpca %>%
  mutate(window = cut(sind, breaks = window3, include.lowest = T)) %>% 
  select(Y, starts_with("pred"), window) %>%
  group_by(window) %>%
  summarise(auc1 = get_auc(Y, pred240),
            auc2 = get_auc(Y, pred480),
            auc3 = get_auc(Y, pred720),
            auc4 = get_auc(Y, pred960),
            auc5 = get_auc(Y, pred1200)) %>%
  filter(window != "[0,240]") %>% 
  select(starts_with("auc"))

colnames(auc_mat_nhanes) <- c("4am", "8am", "12pm", "4pm", "8pm")
```
...
colnames(auc_mat_nhanes2) <- c("4am", "8am", "12pm", "4pm", "8pm")
```

```{r}
data.frame(Window = c("4am-8am", "8am-12pm", "12pm-4pm", "4pm-8pm",
                      "8pm-12am"),
           auc_mat_nhanes, auc_mat_nhanes2, check.names = F) %>%
  kable(digits = 3, table.attr = "style = \"color: black;\"", caption = "Area Under the ROC curve") %>%
  kable_styling(full_width = F) %>% 
  add_header_above(c(" "=1, "fGFPCA" = 5, "GLMMadaptive" = 5)) %>%
  add_header_above(c(" "= 1, "Maximum observation time" = 10))
```

## Figure

```{r fig_pred_anhanes}
rand_id2 <- sample(unique(nhanes_pred_fgfpca$id), 4)

fig_fgfpca_nhanes <- nhanes_pred_fgfpca %>% 
  filter(id %in% rand_id2) %>%
  mutate_at(vars(starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  rename(t=sind) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=pred240, col = "4am"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred480, col = "8am"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred720, col = "12pm"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred960, col = "4pm"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred1200, col = "8pm"), linetype="dashed", na.rm=T)+
  facet_wrap(~id)+
  labs(title = "fGFPCA", col = "Maximum observation time")+
  scale_x_continuous(breaks = window3, 
                     labels = c("Midnight", "4am", "8am", "12pm", "4pm", "8pm", "Midnight"))
```

```{r}
fig_adglmm_nhanes <- nhanes_pred_adglmm %>% 
  filter(id %in% rand_id2) %>%
  mutate_at(vars(starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  mutate(t = t*1440) %>%
  ggplot()+
...
```{r}
ggarrange(fig_fgfpca_nhanes, fig_adglmm_nhanes, nrow = 1, common.legend = T)
```


## Issues

In step 4 of fGFPCA, some eigenvalues, after re-evaluation, was very, very small. It also lost its monotone decreasing nature. 

The debias step of fGFPCA also took a very, very long time. 

...
We would like to use a second reference method for predictive performance comparision. This would be a generalized function-on-function regression where the history records are used to predict a equal length of outcome. Formulation as follows:

$$g(E[Y_i(t)]) = \beta_0(t) + \beta_1 (t)Y_i(s),\ s<t$$

In this case, the outcome must be evaluated at the same time points as the covariate. That is, s and t must have the same number of measurement points. For example, if t is (0, 0.2], then s can be either (0.2, 0.4), (0.4, 0.6), (0.6, 0.8) or (0.8, 1).

What to do when the history t is (0, 0.4)? How share we prediction on 0.2 windows including (0.4, 0.6), (0.6, 0.8) or (0.8, 1)? Does this mean we do not utilize the full observed track but instead only take the most recent subtrack (0.2, 0.4)? Such scenarios remains unclear. 

As for the model fitting, I am not sure how to construct a function-on-function regression using bam or gam. While most functions I found in R does not account for the "generalized" part. At first glance, the model framework is the same as a regular functional regression model:

$$g(E[Y_i(t)]) = \sum_{k}\xi_{k}\phi_k(t) + Y_i(t)\sum_{l}\zeta_{l}\phi_l(s),\ s<t$$

so I fit a gam model where history is [0, 0.2) and future is [0.2, 0.4).


```{r data_format, message=FALSE}
load(here("Data/sim_data.RData"))

# Let's start with a one-simulation example
df_m <- sim_data[[1]]
# class(df_m$Y)
# df_m$Y <- as.factor(df_m$Y)
...
# put covariates and outcome together
dfm1_tr <- train_df %>% filter(window==2) %>% 
  rename(Yf=Y, tf=t) %>%
  left_join(train_df %>% filter(window==1) %>% select(id, sind, t, Y) %>% 
...
fitm1 <- bam(Yf ~ s(tf, bs="cc", k=20) + s(tf, bs="cc", k=20, by = Yh) , 
                   family = binomial, data=dfm1_tr, 
                   method = "fREML",
                   discrete = TRUE)
summary(fitm1)
```

However, the prediction result of this model is very wired. Below is the in-sample estimation of latent process. I am not sure if this is because of a wrong design matrix or ignoring the individual random effect. 


```{r}
dfm1_tr$eta_est <- predict(fitm1, newdata = dfm1_tr, type = "link")
dfm1_tr %>% filter(id %in% 1:4) %>% 
  mutate_at(vars(eta_i, eta_est), inv.logit) %>%
  ggplot()+
  geom_line(aes(x=tf, y=eta_i, col = "true"))+
  geom_line(aes(x=tf, y=eta_est, col = "pred"))+
  facet_wrap(~id)
```

Thinking about the two problems above, perhaps I would want to use the following framework instead. This framework doesn't require same density of s and t. But I am not sure how to construct the design matrix for this.

$$g(E[Y_i(t)]) = \beta_0(t) + \int_{s}\beta_1(t, s)Y_i(s), \ s<t$$

or 

$$g(E[Y_i(t)]) = \beta_0(t) + F(Y_i(s), s, t), \ s<t$$

Perhaps I can try using [refund::ff](https://rdrr.io/cran/refund/man/ff.html)

```{r class.source='fold-show', error=TRUE}
library(refund)

th <- unique(dfm1_tr$th)
tf <- unique(dfm1_tr$tf)

Yhmat <- matrix(dfm1_tr$Yh, nrow=N_train, ncol = length(th), byrow = T)

Yhmat2 <- Yhmat[rep(1:N_train, each = length(th)), ]
Yf <- dfm1_tr$Yf
# length(Yf)
# dim(Yhmat2)

fitm1 <- fgam(Yf ~ s(tf, bs="cc", k=10) + af(Yhmat2, argvals=th) , 
                   family = binomial())
# I couldn't get this function to work
# I think for each observation in the future, all of the history is used as predictor
# therefore the whole historical function is repeatedly used for each subject?

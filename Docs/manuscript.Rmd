...
fig_fgfpca <- pred_list_all[[1]] %>%
  ...
  labs(title = "fGFPCA", col = "Maximum observation time", x = "Time", y="")+
...
fig_gfosr_l1 <- pred_list_gfosr_l1[[1]] %>%
  filter(id %in% rand_id) %>%
  mutate_at(vars(eta_i, pred_w1, pred_w2, pred_w3, pred_w4), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=eta_i, col = "True"))+
  geom_line(aes(x=t, y=pred_w1, col = "0.2"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred_w2, col = "0.4"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred_w3, col = "0.6"), linetype="dashed", na.rm=T)+
  geom_line(aes(x=t, y=pred_w4, col = "0.8"), linetype="dashed", na.rm=T)+
  facet_wrap(~id, nrow = 1)+
  labs(title = "GFOSR (L=1)", col = "Maximum observation time",
       x="Time", y="")+
  scale_x_continuous(breaks = seq(0, 1, by = 0.2))+
  theme(axis.text = element_text(size = 5), 
        title = element_text(size = 8))

fig_gfosr_l5 <- pred_list_gfosr_l5[[1]] %>%
  filter(id %in% rand_id) %>%
  mutate_at(vars(eta_i, pred_w1, pred_w2, pred_w3, pred_w4), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  ...
  geom_line(aes(x=t, y=pred_w4, col = "0.8"), linetype="dashed", na.rm=T)+
  facet_wrap(~id, nrow = 1)+
  labs(title = "GFOSR (L=5)", col = "Maximum observation time",
       x="Time", y="")+
  scale_x_continuous(breaks = seq(0, 1, by = 0.2))+
  theme(axis.text = element_text(size = 5), 
        title = element_text(size = 8))
...
```{r fig_sim, fig.height=8, fig.width=5}
ggarrange(fig_fgfpca, fig_adglmm, fig_gfosr_l1, fig_gfosr_l5,
...
```{r}
rbind(mean_ise_fgfpca, mean_ise_adglmm, 
           mean_ise_gfosr_l1, mean_ise_gfosr_l5) %>%
  as_tibble() %>%
  mutate(Window = rep(c("(0.2, 0.4]", "(0.4, 0.6]", 
                        "(0.6, 0.8]", "(0.8, 1.0]"), 4),
         .before=1) %>%
  kable(digits = 3, booktabs=T,
        table.attr="style=\"color:black;\"") %>%
  kable_styling(full_width = F) %>% 
  group_rows(index = c("fGFPCA" = 4, "GLMMadaptive" = 4,
                     "GFOSR (L=1)" = 4, "GFOSR (L=5)" = 4)) %>%
...
```{r}
rbind(mean_auc_fgfpca, mean_auc_adglmm, 
           mean_auc_gfosr_l1, mean_auc_gfosr_l5) %>%
  as_tibble() %>%
  mutate(Window = rep(c("(0.2, 0.4]", "(0.4, 0.6]", 
                        "(0.6, 0.8]", "(0.8, 1.0]"), 4),
         .before=1) %>%
...
```{r fig_pred_subset}
fig_fgfpca2 <- pred_subset_fGFPCA[[1]] %>%
  filter(id %in% rand_id) %>%
...
fig_adglmm2 <- pred_subset_adglmm[[1]] %>% 
  filter(id %in% rand_id) %>%
  mutate_at(vars(eta_i, starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
...
fig_gfosr_l1_2 <- pred_subset_gfosr_l1[[1]] %>% 
  filter(id %in% rand_id) %>%
  mutate_at(vars(eta_i, starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
...
fig_gfosr_l5_2 <- pred_subset_gfosr_l5[[1]] %>% 
  filter(id %in% rand_id) %>%
  mutate_at(vars(eta_i, starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
...
```{r fig_sim2, fig.height=8, fig.width=5}
ggarrange(fig_fgfpca2, fig_adglmm2, fig_gfosr_l1_2, fig_gfosr_l5_2,
          ncol = 1, common.legend = T)
```
...
```{r}
rbind(mean_ise_fgfpca2, mean_ise_adglmm2, 
           mean_ise_gfosr_l1_2, mean_ise_gfosr_l5_2) %>%
  as_tibble() %>%
  mutate(Window = rep(c("(0.2, 0.4]", "(0.4, 0.6]", 
                        "(0.6, 0.8]", "(0.8, 1.0]"), 4),
         .before=1) %>%
...
```{r}
rbind(mean_auc_fgfpca2, mean_auc_adglmm2, 
           mean_auc_gfosr_l1_2, mean_auc_gfosr_l5_2) %>%
  as_tibble() %>%
  mutate(Window = rep(c("(0.2, 0.4]", "(0.4, 0.6]", 
                        "(0.6, 0.8]", "(0.8, 1.0]"), 4),
         .before=1) %>%

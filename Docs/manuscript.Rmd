...
df_m <- sim_data[[1]]
df_m %>% filter(t>0.2 & t<=0.4)
```

We would like to use a second reference method for predictive performance comparision. In this madel, predictions on a specific interval are all made using the last few observations in the observed window. For example, if we are given observations on [0, 0.2] (j = 1...200), we may use L observations taken right before t=0.2($t_j: j=200, ... 200-(L-1)$) as time-fixed covariate to predict any future time. 
...
Under this framework, for each dataset we need to fit four models: 

...
data.frame(Window = c("(0.35,0.45]", "(0.45,0.55]", "(0.55,0.65]", "(0.65, 0.75]"),
  ...
  kable(digits = 3, caption = "Integrated squared error") %>%
  kable_styling(full_width = F) %>% 
  add_header_above(c(" "=1, "fGFPCA" = 4, "GLMMadaptive" = 4)) %>%
...
```{r small_exp}
# a smaller example of binary functions
df_nhanes %>% select(SEQN, Z, sind) %>% 
  filter(SEQN %in% rand_id2 & Z == 1) %>% 
  mutate(SEQN = as.factor(SEQN)) %>% 
  ggplot()+
  geom_point(aes(x=sind, y = SEQN), size = 0.5)+
  labs(x="Time", y="Subject", title = "NHANES binary activity indicator")
```
...
```{r binning}
df_nhanes %>% select(SEQN, Z, sind) %>% 
  filter(SEQN %in% rand_id2 & sind >= 650 & sind <= 750) %>% 
  mutate(SEQN = as.factor(SEQN)) %>% 
  ggplot()+

...
fig_adglmm_subset <- df_subset_adglmm %>% filter(id %in% rand_id2) %>%
  mutate_at(vars(eta_i, starts_with("pred")), function(x){exp(x)/(1+exp(x))}) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
  geom_line(aes(x=t, y=eta_i, col = "True"))+
...
## ISE container 
ise_mat_subset <- ise_mat_subset_ref <- 
  array(NA, dim = c(length(window)-2, length(window)-2, M2))
```
...
ise_mat_subset2 <- df_subset_adglmm %>%
    mutate(err1 = (pred0.2-eta_i)^2,
           err2 = (pred0.4-eta_i)^2,
           err3 = (pred0.6-eta_i)^2,
           err4 = (pred0.8-eta_i)^2) %>%
    ...
    filter(window != "[0,0.2]") %>% 
    select(starts_with("err"))
colnames(ise_mat_subset2) <- c("0.2", "0.4", "0.6", "0.8")
```
...
```{r}
## auc container 
auc_mat_subset <- array(NA, dim = c(length(window)-2, length(window)-2, M2))
auc_mat_subset_ref <- array(NA, dim = c(length(window)-2, length(window)-2, M2))
```
...
auc_mat_subset2 <- df_subset_adglmm %>%
  mutate(window = cut(t, breaks = window, include.lowest = T)) %>% 
  select(Y, starts_with("pred"), window) %>%
  group_by(window) %>%
  summarise(auc1 = get_auc(Y, pred0.2),
...
colnames(auc_mat_subset2) <- c("0.2", "0.4", "0.6", "0.8")
```

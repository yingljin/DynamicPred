...
bind_rows(sim1 %>% mutate(sim = "Original Sample"), sim2 %>% mutate(sim = "Reduced sample")) %>%
  ggplot()+
  geom_point(aes(x=t, y=Y), size = 0.2)+
...
df_nhanes %>% 
  group_by(SEQN) %>%
  summarise(n0 = sum(Z==0)) %>%
  filter(n0==1440) %>%
  select(SEQN) %>% unlist()
```


```{r load_nhanes_result, eval=F}
# df_nhanes <- read_rds(here("Data/nhanes_bi.rds"))
...
```{r fig_appl_paper, fig.height=4, fig.width=10, eval=F}

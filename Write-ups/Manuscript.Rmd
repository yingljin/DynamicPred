...
According to the Karhunen-Loève theorem, the unobserved random effect function $b_i(t)$ can be represented as the linear combination of a infinite set of orthogonal functions: $b_i(t)=\sum_{k=1}^{\infty}\xi_{ik}\phi_{k}(t)$. We approximate this function by a finite set of $\phi_{k}(t)$, such that $b_i(t)=\sum_{k=1}^{K}\xi_{ik}\phi_{k}(t)+\epsilon_i(t)$, where $\epsilon_i(t)$ indicates random noise introduced by this approximation. This random noise is assumed to follow a zero-mean Gaussian process, such that at each specific point t, $\epsilon_i(t) \sim N(0, \sigma^2)$.
